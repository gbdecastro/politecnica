...
function horasGrupos() {
    //-------------
    //- Horas por Grupos - Bar Chart
    //-------------
    var ctx = document.getElementById("horas_grupos").getContext('2d');
    var chart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"],
        ...
        options: {
            tooltips: {
                ...
                callbacks: {
                    label: function label(tooltipItem, data) {
                        var label = data.labels[tooltipItem.index];
                        return formatoHoras(tooltipItem.yLabel);
                    }
                }
            },
            scales: {
    ...
    $.ajax({
        ...
        $.each(data, function (i, entry) {
            ...
            if (id_grupo != id_grupo_aux) {
                ...
                chart.data.datasets[cont].data.push(entry.nb_horas);
            }
        });
        chart.update();
        $('.overlay').remove();
